...
void forceContactECM(Particle particles[], double network[][NUMBER])
{
    int i, j;
    ...
    for (i = 0; i < NUMBER; i++)
    {
        for (j = 0; j < NUMBER; j++)
        {
            ...
            if (isOppositeMove(particles[i], particles[j]) & (i != j))
            {
                ...
                //std::cout << "force yes!" << i << "," << j << std::endl;
                double d = boundaryDistance(particles[i], particles[j]);
                double deltax, deltay;
                deltax = boundaryDisplacementX(particles[i], particles[j]);
                deltay = boundaryDisplacementY(particles[i], particles[j]);
                ...
                network[i][j] = sqrt(particles[i].force[0] * particles[i].force[0] + particles[i].force[1] * particles[i].force[1]);
            }
        }
    }
}
...
void simulation(int simID)
{
    ...
    */
    ...
    for (int step = 0; step < STEP; step++)
    {
        ...
        double network[NUMBER][NUMBER] = {0};
        forceContactECM(particles, network);
        //forceSelf(particles);
        ...
        {
            //std::cout << "Step " << step + 1 << " out of " << STEP << std::endl;
            const std::string fileName = "data" + std::to_string(simID) + "/network" + std::to_string(step + 1) + ".txt";
            ...
            for (auto p = std::begin(network); p != std::end(network); ++p)
            {
